...
struct register_t
{
  ...
  operator std::uint32_t() const { return data; }
...
struct function_t
{
  ...
  operator std::uint32_t() { return reinterpret_cast<std::uint32_t>(address); }

  operator void*() { return reinterpret_cast<void*>(address); }
...
exception_object&
get_exception_object(void* p_thrown_exception);

inline exception_object&
extract_exception_object(void* p_thrown_exception)
{
...
inline void*
extract_thrown_object(void* p_exception_object)
{
...
restore_cpu_core(ke::cortex_m_cpu& p_cpu_core)
{
  asm volatile(
    "ldr r0, [%[regs], #0]\n" // Load R0
    "ldr r1, [%[regs], #4]\n" // Load R1
...
to_absolute_address(const void* p_object)
{
...
[[gnu::used]] inline std::uint32_t
runtime_to_absolute_address(const void* p_object)
{
  return to_absolute_address(p_object);
}

inline std::uint32_t*
to_absolute_address_ptr(const void* p_object)
{
