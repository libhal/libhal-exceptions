#include <array>
#include <cstdint>
#include <expected>
#include <string_view>
#include <tuple>
#include <utility>

// External functions
extern void start();
extern void end();
extern void log_start(std::string_view);

// Global side effect to prevent optimization
std::int32_t volatile side_effect = 0;

// Error type for testing
struct test_error
{
  std::array<std::uint8_t, 4> data;
};

using result_type = std::expected<int, test_error>;

// Simple class without destructor
class simple_object
{
public:
  explicit simple_object(std::int32_t value)
    : m_value(value)
  {
    side_effect = side_effect + 1;
  }

  void do_work()
  {
    side_effect = side_effect + m_value;
  }

private:
  std::int32_t m_value;
};

// Class with destructor for testing destructor impact
class destructor_object
{
public:
  explicit destructor_object(std::int32_t value)
    : m_value(value)
  {
    side_effect = side_effect + 1;
  }

  ~destructor_object()
  {
    side_effect = side_effect + 1;
  }

  void do_work()
  {
    side_effect = side_effect + m_value;
  }

private:
  std::int32_t m_value;
};

__attribute__((noinline)) result_type depth_50();
__attribute__((noinline)) result_type depth_49();
__attribute__((noinline)) result_type depth_48();
__attribute__((noinline)) result_type depth_47();
__attribute__((noinline)) result_type depth_46();
__attribute__((noinline)) result_type depth_45();
__attribute__((noinline)) result_type depth_44();
__attribute__((noinline)) result_type depth_43();
__attribute__((noinline)) result_type depth_42();
__attribute__((noinline)) result_type depth_41();
__attribute__((noinline)) result_type depth_40();
__attribute__((noinline)) result_type depth_39();
__attribute__((noinline)) result_type depth_38();
__attribute__((noinline)) result_type depth_37();
__attribute__((noinline)) result_type depth_36();
__attribute__((noinline)) result_type depth_35();
__attribute__((noinline)) result_type depth_34();
__attribute__((noinline)) result_type depth_33();
__attribute__((noinline)) result_type depth_32();
__attribute__((noinline)) result_type depth_31();
__attribute__((noinline)) result_type depth_30();
__attribute__((noinline)) result_type depth_29();
__attribute__((noinline)) result_type depth_28();
__attribute__((noinline)) result_type depth_27();
__attribute__((noinline)) result_type depth_26();
__attribute__((noinline)) result_type depth_25();
__attribute__((noinline)) result_type depth_24();
__attribute__((noinline)) result_type depth_23();
__attribute__((noinline)) result_type depth_22();
__attribute__((noinline)) result_type depth_21();
__attribute__((noinline)) result_type depth_20();
__attribute__((noinline)) result_type depth_19();
__attribute__((noinline)) result_type depth_18();
__attribute__((noinline)) result_type depth_17();
__attribute__((noinline)) result_type depth_16();
__attribute__((noinline)) result_type depth_15();
__attribute__((noinline)) result_type depth_14();
__attribute__((noinline)) result_type depth_13();
__attribute__((noinline)) result_type depth_12();
__attribute__((noinline)) result_type depth_11();
__attribute__((noinline)) result_type depth_10();
__attribute__((noinline)) result_type depth_09();
__attribute__((noinline)) result_type depth_08();
__attribute__((noinline)) result_type depth_07();
__attribute__((noinline)) result_type depth_06();
__attribute__((noinline)) result_type depth_05();
__attribute__((noinline)) result_type depth_04();
__attribute__((noinline)) result_type depth_03();
__attribute__((noinline)) result_type depth_02();
__attribute__((noinline)) result_type depth_01();

__attribute__((noinline)) result_type depth_50()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_49();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_49()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_48();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_48()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_47();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_47()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_46();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_46()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_45();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_45()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_44();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_44()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_43();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_43()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_42();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_42()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_41();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_41()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_40();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_40()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_39();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_39()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_38();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_38()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_37();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_37()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_36();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_36()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_35();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_35()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_34();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_34()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_33();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_33()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_32();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_32()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_31();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_31()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_30();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_30()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_29();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_29()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_28();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_28()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_27();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_27()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_26();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_26()
{
  simple_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_25();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_25()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_24();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_24()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_23();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_23()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_22();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_22()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_21();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_21()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_20();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_20()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_19();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_19()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_18();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_18()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_17();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_17()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_16();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_16()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_15();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_15()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_14();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_14()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_13();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_13()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_12();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_12()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_11();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_11()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_10();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_10()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_09();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_09()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_08();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_08()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_07();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_07()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_06();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_06()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_05();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_05()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_04();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_04()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_03();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_03()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_02();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_02()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();
  auto result = depth_01();
  if (!result) {
    return result;  // Propagate error
  }
  return result.value();
}

result_type depth_01()
{
  destructor_object obj(side_effect >> 8);
  obj.do_work();

  // This is where the error originates
  if (side_effect > 0) {
    start();
    return std::unexpected(test_error{ .data = { 0xDE, 0xAD, 0xBE, 0xEF } });
  }

  return side_effect;
}

std::array function_addresses = {
  reinterpret_cast<void*>(&depth_01), reinterpret_cast<void*>(&depth_02),
  reinterpret_cast<void*>(&depth_03), reinterpret_cast<void*>(&depth_04),
  reinterpret_cast<void*>(&depth_05), reinterpret_cast<void*>(&depth_06),
  reinterpret_cast<void*>(&depth_07),
};

// Test runner
void run_test()
{
  log_start("RESULT_50PCT_DEPTH50");
  side_effect = 1;  // Ensure we will return error

  auto result = depth_50();
  if (!result) {
    end();
    if (side_effect == 0) {
      std::ignore = function_addresses[side_effect];
    }
  }
}
